...
int applyThrottleOffset(int throttleValue) {
  // Center the throttle range
  int centeredThrottle = throttleValue - 127;
  
  // Apply deadzone if needed (adjust the 5 to increase/decrease deadzone)
  if (abs(centeredThrottle) < 5) {
    return 127;  // Center position
  }
  
  // Invert the direction and scale to 0-255 range
  return map(centeredThrottle, -127, 127, 191, 63);
}
