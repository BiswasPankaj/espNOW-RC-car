...
typedef struct PacketData
{
  ...
  byte steering;          //ch1
  byte knob1;             //ch3
  byte toggle;            //ch4 (new field for toggle switch)
} PacketData;
...
Servo ch2;  //throttle
Servo ch3;  //knob1       
int togglePin = 14;  // Digital pin for toggle switch output
...
void setInputDefaultValues()
{
  ...
  receiverData.toggle = 0;  // Default value for toggle switch
}
...
void mapAndWriteValues()
{
  ...
  digitalWrite(togglePin, receiverData.toggle > 127 ? HIGH : LOW);  // Set toggle switch output
}
...
void OnDataRecv(const esp_now_recv_info_t *esp_now_info, const uint8_t *incomingData, int len) 
{
  ...
  String inputData = "Recv from: " + String(macStr) + " values " + 
                     ...
                     String(receiverData.toggle);  // Added toggle switch value
...
void setUpPinModes()
{
  ...
  pinMode(togglePin, OUTPUT);       // Set toggle switch pin as output
